...
.
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_failed TYPE TABLE FOR FAILED ZJS_I_TRAVEL_U_D.
    TYPES tt_travel_reported TYPE TABLE FOR REPORTED ZJS_I_TRAVEL_U_D.
...
********************************************************************************
  METHOD get_instance_features.
    READ ENTITIES OF ZJS_I_TRAVEL_U_D IN LOCAL MODE
    ENTITY Travel
...
**********************************************************************
  METHOD create.
    DATA: messages   TYPE zjs_tt_message,
          travel_in  TYPE zjs_travel,
          travel_out TYPE zjs_travel.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_create>).
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_CREATE'
        EXPORTING
          is_travel         = CORRESPONDING zjs_st_travel_in( travel_in )
...
**********************************************************************
  METHOD update.
    DATA: messages TYPE zjs_tt_message,
          travel   TYPE zjs_travel,
          travelx  TYPE zjs_st_travel_inx. "refers to x structure (> BAPIs)
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_update>).
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          is_travel   = CORRESPONDING zjs_st_travel_in( travel )
          is_travelx  = travelx
        IMPORTING
          et_messages = messages.
...
**********************************************************************
  METHOD delete.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<travel_delete>).
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_DELETE'
        EXPORTING
...
**********************************************************************
  METHOD lock.
    TRY.
        ...
        DATA(lock) = cl_abap_lock_object_factory=>get_instance( iv_name = 'EZ_JS_ETRAVEL' ).
...
**********************************************************************
  METHOD rba_Booking.
    DATA: travel_out  TYPE zjs_travel,
          booking_out TYPE zjs_tt_booking,
...
**********************************************************************
  METHOD cba_Booking.
    DATA: messages        TYPE zjs_tt_message,
          booking_old     TYPE zjs_tt_booking,
          booking         TYPE zjs_booking,
          last_booking_id TYPE zjs_de_booking_id VALUE '0'.
    ...
    LOOP AT entities_cba ASSIGNING FIELD-SYMBOL(<travel>).
      ...
      ELSE.
        ...
        LOOP AT <travel>-%target ASSIGNING FIELD-SYMBOL(<booking_create>).
          ...
          CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_UPDATE'
            EXPORTING
              is_travel   = VALUE zjs_st_travel_in( travel_id = travelid )
              is_travelx  = VALUE zjs_st_travel_inx( travel_id = travelid )
              it_booking  = VALUE zjs_tt_booking_in( ( CORRESPONDING #( booking ) ) )
              it_bookingx = VALUE zjs_tt_booking_inx(
              (
...
**********************************************************************
  METHOD set_status_booked.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<travel_set_status_booked>).
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_SET_BOOKING'
        EXPORTING
          iv_travel_id = travelid
        IMPORTING
          et_messages  = messages.
...
CLASS lsc_I_TRAVEL_U IMPLEMENTATION.
  ...
  METHOD adjust_numbers.
    ...
    DATA: travel_mapping       TYPE zjs_if_flight_legacy=>tt_ln_travel_mapping,
    ...
    CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_ADJ_NUMBERS'
      IMPORTING
  ...
  METHOD save.
    CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_SAVE'.
  ENDMETHOD.

  METHOD cleanup.
    CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_INITIALIZE'.
  ENDMETHOD.
