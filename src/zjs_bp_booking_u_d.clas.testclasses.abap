...
RISK LEVEL HARMLESS.
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      ...
      booking_supplement_id TYPE zjs_de_booking_supplement_id VALUE '21',
      supplement_id         TYPE zjs_de_supplement_id VALUE 'XX-42'.
    ...
    METHODS:
      ...
      "! Checks if { @link ..lhc_booking.METH:read } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds and the <em>Booking_ID</em> is found.
      read_success                  FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:read } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds but the <em>Booking_ID</em> to be read is not found.
      read_wrong_booking_id         FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:read } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      read_fail                     FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_booking.METH:update } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } succeeds.
      update_success                FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:update } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } fails.
      update_fail                   FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_booking.METH:delete } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } succeeds.
      delete_success                FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:delete } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } fails.
      delete_fail                   FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_booking.METH:rba_Travel } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds.
      rba_travel_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:rba_Travel } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      rba_travel_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_booking.METH:rba_Booksupplement } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds.
      rba_bookingsupplement_success FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:rba_Booksupplement } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      rba_bookingsupplement_fail    FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_booking.METH:cba_booksupplement } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } and { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } both succeeds.
      cba_success                   FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:cba_booksupplement } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      cba_fail_read                 FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_booking.METH:cba_booksupplement } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds but { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } fails.
...
CLASS ltcl_handler IMPLEMENTATION.
  ...
  METHOD read_success.
    DATA:
      keys        TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\Booking,
      result      TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking,
      result_line TYPE STRUCTURE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking,
  ...
  METHOD update_success.
    DATA:
    entities TYPE TABLE FOR UPDATE ZJS_I_TRAVEL_U_D\\booking.

    DATA(fm_update_double) = fm_test_environment->get_double( fm_update ).

    DATA(travel) = VALUE zjs_st_travel_in( travel_id = travel_id ).
    DATA(travelx) = VALUE zjs_st_travel_inx( travel_id = travel_id ).
    DATA(bookings) = VALUE zjs_tt_booking_in( (
    travel_id = travel_id
    booking_id = booking_id
    customer_id = customer_id
    carrier_id = carrier_id
    ) ).
    DATA(bookingsx) = VALUE zjs_tt_booking_inx( (
  ...
  METHOD delete_success.
    ...
    DATA(travel) = VALUE zjs_st_travel_in( travel_id = travel_id ).
    DATA(travelx) = VALUE zjs_st_travel_inx( travel_id = travel_id ).
    DATA(bookings) = VALUE zjs_tt_booking_in( ( travel_id = travel_id booking_id = booking_id ) ).
    DATA(bookingsx) = VALUE zjs_tt_booking_inx( ( action_code = zjs_if_flight_legacy=>action_code-delete booking_id = booking_id ) ).
  ...
  METHOD rba_travel_success.
    DATA:
      keys                   TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\Booking\_Travel,
      result                 TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking\_Travel,
      result_line            TYPE STRUCTURE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking\_Travel,
      association_links      TYPE TABLE FOR READ LINK ZJS_I_TRAVEL_U_D\\Booking\_Travel,
      association_links_line TYPE STRUCTURE FOR READ LINK ZJS_I_TRAVEL_U_D\\Booking\_Travel.
  ...
  METHOD rba_bookingsupplement_success.
    DATA:
      keys                   TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      result                 TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      result_line            TYPE STRUCTURE FOR READ RESULT ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      association_links      TYPE TABLE FOR READ LINK ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      association_links_line TYPE STRUCTURE FOR READ LINK ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      booking_supplements    TYPE zjs_tt_booking_supplement.
    ...
    DATA(booking_supplement) = VALUE zjs_book_suppl(
    travel_id = travel_id
    booking_id = booking_id
  ...
  METHOD cba_success.
    DATA:
      entities_cba                   TYPE TABLE FOR CREATE ZJS_I_TRAVEL_U_D\\Booking\_BookSupplement,
      ...
      old_booking_supplement         TYPE zjs_book_suppl,
      new_booking_supplement         TYPE zjs_st_booking_supplement_in,
      new_booking_supplementx        TYPE zjs_st_booking_supplement_inx,
      old_booking_supplements        TYPE zjs_tt_booking_supplement,
      new_booking_supplements        TYPE zjs_tt_booking_supplement_in,
      new_booking_supplementsx       TYPE zjs_tt_booking_supplement_inx.

    travel = VALUE zjs_st_travel_in( travel_id = travel_id ).
    travelx = VALUE zjs_st_travel_inx( travel_id = travel_id ).

    new_booking_supplement = VALUE zjs_st_booking_supplement_in(
    booking_id = booking_id
    ...
    old_booking_supplement = VALUE zjs_book_suppl(
    travel_id = travel_id
    booking_id = booking_id
    booking_supplement_id = booking_supplement_id
    supplement_id = supplement_id
    ...
    new_booking_supplementx = VALUE zjs_st_booking_inx(
    booking_id = booking_id
