...
.
  PRIVATE SECTION.
    ...
    TYPES tt_booking_failed TYPE TABLE FOR FAILED ZJS_I_BOOKING_U_D.
    TYPES tt_booking_reported TYPE TABLE FOR REPORTED ZJS_I_BOOKING_U_D.

    TYPES tt_bookingsupplement_failed TYPE TABLE FOR FAILED ZJS_I_BOOKINGSUPPLEMENT_U_D.
    TYPES tt_bookingsupplement_reported TYPE TABLE FOR REPORTED ZJS_I_BOOKINGSUPPLEMENT_U_D.
...
**********************************************************************
  METHOD update.
    DATA: messages TYPE zjs_tt_message,
          booking  TYPE zjs_booking,
          bookingx TYPE zjs_st_booking_inx.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      bookingx-action_code = zjs_if_flight_legacy=>action_code-update.

      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          is_travel   = VALUE zjs_st_travel_in( travel_id = <booking>-travelid )
          is_travelx  = VALUE zjs_st_travel_inx( travel_id = <booking>-travelid )
          it_booking  = VALUE zjs_tt_booking_in( ( CORRESPONDING #( booking ) ) )
          it_bookingx = VALUE zjs_tt_booking_inx( ( bookingx ) )
        IMPORTING
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          is_travel   = VALUE zjs_st_travel_in( travel_id = <booking>-travelid )
          is_travelx  = VALUE zjs_st_travel_inx( travel_id = <booking>-travelid )
          it_booking  = VALUE zjs_tt_booking_in( ( booking_id = <booking>-bookingid ) )
          it_bookingx = VALUE zjs_tt_booking_inx( ( booking_id = <booking>-bookingid
...
**********************************************************************
  METHOD read.
    DATA: travel_out   TYPE zjs_travel,
          bookings_out TYPE zjs_tt_booking,
          messages     TYPE zjs_tt_message.
    ...
    GROUP BY <booking_by_travel>-travelid .
      ...
      CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_READ'
        EXPORTING
...
***********************************************************************
  METHOD rba_Booksupplement.
    DATA: bookingsupplements_out TYPE zjs_tt_booking_supplement,
          messages               TYPE zjs_tt_message.
...
***********************************************************************
  METHOD cba_Booksupplement.
    DATA: message                   TYPE LINE OF zjs_tt_message,
          messages                  TYPE zjs_tt_message,
          booksupplements_old       TYPE zjs_tt_booking_supplement,
          booksupplement            TYPE zjs_book_suppl,
          last_bookingsupplement_id TYPE zjs_de_booking_supplement_id.
    ...
    LOOP AT entities_cba ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      ELSE.
        ...
        last_bookingsupplement_id = REDUCE #( INIT res TYPE zjs_de_booking_supplement_id
        ...
        LOOP AT <booking>-%target ASSIGNING FIELD-SYMBOL(<supplement>).
          ...
          CALL FUNCTION 'ZJS_FLIGHT_TRAVEL_UPDATE'
            EXPORTING
              is_travel              = VALUE zjs_st_travel_in( travel_id = parent_key-travelid )
              is_travelx             = VALUE zjs_st_travel_inx( travel_id = parent_key-travelid )
              it_booking_supplement  = VALUE zjs_tt_booking_supplement_in( ( CORRESPONDING #( booksupplement ) ) )
              it_booking_supplementx = VALUE zjs_tt_booking_supplement_inx( ( VALUE #(
  ...
  METHOD map_messages.
    ...
    LOOP AT messages INTO DATA(message).
      IF message-msgty = 'E' OR message-msgty = 'A'.
        ...
        %fail-cause = zjs_cl_travel_auxiliary=>get_cause_from_message(
        msgid = message-msgid
        msgno = message-msgno
        ) )
        TO failed.
