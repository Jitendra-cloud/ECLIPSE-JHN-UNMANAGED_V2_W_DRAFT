...
RISK LEVEL HARMLESS.
  ...
  PRIVATE SECTION.
    CONSTANTS:
      fm_create      TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_CREATE',
      fm_delete      TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_DELETE',
      fm_read        TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_READ',
      fm_set_booking TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_SET_BOOKING',
      fm_update      TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_UPDATE'.

    CONSTANTS:
      cid         TYPE abp_behv_cid VALUE '42',
      travel_id   TYPE zjs_de_travel_id VALUE '1337',
      agency_id   TYPE zjs_de_agency_id VALUE '42',
      customer_id TYPE zjs_de_customer_id VALUE '123',
      booking_id  TYPE zjs_de_booking_id VALUE '20',
      carrier_id  TYPE zjs_de_carrier_id VALUE 'XX'.
    ...
    DATA:
      ...
      mapped             TYPE RESPONSE FOR MAPPED EARLY ZJS_I_TRAVEL_U_D,
      failed             TYPE RESPONSE FOR FAILED EARLY ZJS_I_TRAVEL_U_D,
      reported           TYPE RESPONSE FOR REPORTED EARLY ZJS_I_TRAVEL_U_D,
    ...
    METHODS:
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_CREATE } succeeds.
      create_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_CREATE } fails.
      create_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } succeeds.
      update_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } fails.
      update_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_DELETE } succeeds.
      delete_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_DELETE } fails.
      delete_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds.
      read_success              FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      read_fail                 FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds.
      rba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      rba_fail                  FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } and { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } both succeeds.
      cba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } fails.
      cba_fail_read             FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_READ } succeeds but { @link FUNC:ZJS_FLIGHT_TRAVEL_UPDATE } fails.
      cba_fail_update           FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_SET_BOOKING } succeeds.
      set_status_booked_success FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZJS_FLIGHT_TRAVEL_SET_BOOKING } fails.
...
CLASS ltcl_handler IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    messages = VALUE zjs_tt_message( ( message ) ).
  ...
  METHOD create_success.
    DATA:
    entities TYPE TABLE FOR CREATE ZJS_I_TRAVEL_U_D\\travel.


    DATA(fm_create_double) = fm_test_environment->get_double( fm_create ).

    DATA(travel) = VALUE zjs_travel( travel_id = travel_id ).
  ...
  METHOD update_success.
    DATA:
    entities TYPE TABLE FOR UPDATE ZJS_I_TRAVEL_U_D\\travel.

    DATA(fm_update_double) = fm_test_environment->get_double( fm_update ).

    DATA(travel) = VALUE zjs_st_travel_in( travel_id = travel_id agency_id = agency_id customer_id = customer_id ).
    DATA(travelx) = VALUE zjs_st_travel_inx( travel_id = travel_id agency_id = abap_true customer_id = abap_true ).
  ...
  METHOD delete_success.
    DATA:
    keys TYPE TABLE FOR DELETE ZJS_I_TRAVEL_U_D\\travel.
  ...
  METHOD read_success.
    DATA:
      keys        TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\travel,
      result      TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\travel,
      result_line TYPE STRUCTURE FOR READ RESULT ZJS_I_TRAVEL_U_D\\travel.

    DATA(travel) = VALUE zjs_travel(
    travel_id = travel_id
    agency_id = agency_id
  ...
  METHOD rba_success.
    DATA:
      keys_rba               TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\travel\_Booking,
      result                 TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\travel\_Booking,
      result_line            TYPE STRUCTURE FOR READ RESULT ZJS_I_TRAVEL_U_D\\travel\_Booking,
      association_links      TYPE TABLE FOR READ LINK ZJS_I_TRAVEL_U_D\\travel\_Booking,
      association_links_line TYPE STRUCTURE FOR READ LINK ZJS_I_TRAVEL_U_D\\travel\_Booking,
      bookings               TYPE zjs_tt_booking.
    ...
    DATA(booking) = VALUE zjs_booking(
    travel_id = travel_id
  ...
  METHOD cba_success.
    DATA:
      entities_cba        TYPE TABLE FOR CREATE ZJS_I_TRAVEL_U_D\\travel\_booking,
      mapped_line_booking LIKE LINE OF mapped-booking,
      travel              TYPE zjs_st_travel_in,
      travelx             TYPE zjs_st_travel_inx,
      old_booking         TYPE zjs_booking,
      new_booking         TYPE zjs_st_booking_in,
      new_bookingx        TYPE zjs_st_booking_inx,
      old_bookings        TYPE zjs_tt_booking,
      new_bookings        TYPE zjs_tt_booking_in,
      new_bookingsx       TYPE zjs_tt_booking_inx.

    travel = VALUE zjs_st_travel_in( travel_id = travel_id ).
    travelx = VALUE zjs_st_travel_inx( travel_id = travel_id ).

    new_booking = VALUE zjs_st_booking_in(
    travel_id = travel_id
    ...
    new_bookingx = VALUE zjs_st_booking_inx(
    booking_id = booking_id
  ...
  METHOD cba_fail_read.
    ...
    DATA(new_booking) = VALUE zjs_st_booking_in(
    travel_id = travel_id
    booking_id = booking_id + 1
    customer_id = customer_id + 1
    carrier_id = carrier_id
  ...
  METHOD get_instance_feature.
    TYPES:
      BEGIN OF check_line,
        travel_id                     TYPE zjs_travel-travel_id,
        status                        TYPE zjs_travel-status,
    ...
    DATA:
      keys               TYPE TABLE FOR INSTANCE FEATURES KEY ZJS_I_TRAVEL_U_D\\travel,
      requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST ZJS_I_TRAVEL_U_D\\travel,
      result             TYPE TABLE FOR INSTANCE FEATURES RESULT ZJS_I_TRAVEL_U_D\\travel,
      result_line        TYPE STRUCTURE FOR INSTANCE FEATURES RESULT ZJS_I_TRAVEL_U_D\\travel,
      import_read        TYPE STRUCTURE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\travel,
      import_read_tab    TYPE TABLE FOR READ IMPORT ZJS_I_TRAVEL_U_D\\travel,
      result_read        TYPE TABLE FOR READ RESULT ZJS_I_TRAVEL_U_D\\travel.
...
* ).
* DATA(env_config) = cl_abap_behv_test_environment=>create_environment_config( )->set_bdef_dependencies( bdef_dependencies = VALUE #( ( 'ZJS_I_TRAVEL_U_D' ) ) ).
* DATA(bo_double_environment) = cl_abap_behv_test_environment=>create( environment_config = env_config ).
*
* DATA(bo_double) = bo_double_environment->get_test_double( 'ZJS_I_TRAVEL_U_D' ).
*
...
* data(bo_double_input) = bo_double->create_read_input_for_entity( 'ZJS_I_TRAVEL_U_D' "can accept the entity name
...
* ENDLOOP.
  ...
  METHOD set_status_booked_success.
    DATA:
      result      TYPE TABLE FOR ACTION RESULT ZJS_I_TRAVEL_U_D\\travel~set_status_booked,
      result_line TYPE STRUCTURE FOR ACTION RESULT ZJS_I_TRAVEL_U_D\\travel~set_status_booked.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZJS_I_TRAVEL_U_D\\travel,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZJS_I_TRAVEL_U_D\\travel.
...
RISK LEVEL HARMLESS.
  ...
  PRIVATE SECTION.
    CONSTANTS:
      fm_initialize     TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_INITIALIZE',
      fm_adjust_numbers TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_ADJ_NUMBERS',
      fm_save           TYPE sxco_fm_name VALUE 'ZJS_FLIGHT_TRAVEL_SAVE'.
    ...
    DATA:
      ...
      mapped       TYPE RESPONSE FOR MAPPED LATE ZJS_I_TRAVEL_U_D,
      failed       TYPE RESPONSE FOR FAILED LATE ZJS_I_TRAVEL_U_D,
      reported     TYPE RESPONSE FOR REPORTED LATE ZJS_I_TRAVEL_U_D.
    ...
    METHODS:
      ...
      "! Checks if { @link ..lsc_i_travel_u.METH:adjust_numbers } mapping from { @link FUNC:ZJS_FLIGHT_TRAVEL_ADJ_NUMBERS } works as expected.
      adjust_numbers    FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:save } calls { @link FUNC:ZJS_FLIGHT_TRAVEL_SAVE } and does not raise any exception.
      save              FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:cleanup } calls { @link FUNC:ZJS_FLIGHT_TRAVEL_INITIALIZE } and does not raise any exception.
...
* mapped = mapped
  ...
  METHOD adjust_numbers.
    CONSTANTS:
      travel_pre_1               TYPE zjs_de_travel_id VALUE '9990',
      travel_pre_2               TYPE zjs_de_travel_id VALUE '9995',
      travel_final_1             TYPE zjs_de_travel_id VALUE '6310',
      travel_final_2             TYPE zjs_de_travel_id VALUE '6315',
      booking_pre_1              TYPE zjs_de_booking_id VALUE '990',
      booking_pre_2              TYPE zjs_de_booking_id VALUE '995',
      booking_final_1            TYPE zjs_de_booking_id VALUE '410',
      booking_final_2            TYPE zjs_de_booking_id VALUE '415',
      booking_supplement_pre_1   TYPE zjs_de_booking_supplement_id VALUE '90',
      booking_supplement_pre_2   TYPE zjs_de_booking_supplement_id VALUE '95',
      booking_supplement_final_1 TYPE zjs_de_booking_supplement_id VALUE '10',
      booking_supplement_final_2 TYPE zjs_de_booking_supplement_id VALUE '15'.


    DATA:
      travel_mapping       TYPE zjs_if_flight_legacy=>tt_ln_travel_mapping,
      booking_mapping      TYPE zjs_if_flight_legacy=>tt_ln_booking_mapping,
      bookingsuppl_mapping TYPE zjs_if_flight_legacy=>tt_ln_bookingsuppl_mapping.

    travel_mapping = VALUE zjs_if_flight_legacy=>tt_ln_travel_mapping(
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_travel( travel_id = travel_pre_1 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_travel( travel_id = travel_final_1 ) )
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_travel( travel_id = travel_pre_2 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_travel( travel_id = travel_final_2 ) )
    ).

    booking_mapping = VALUE zjs_if_flight_legacy=>tt_ln_booking_mapping(
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1 booking_id = booking_pre_1 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_booking( travel_id = travel_final_1 booking_id = booking_final_1 ) )
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1 booking_id = booking_pre_2 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_booking( travel_id = travel_final_1 booking_id = booking_final_2 ) )
    ).

    bookingsuppl_mapping = VALUE zjs_if_flight_legacy=>tt_ln_bookingsuppl_mapping(
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1 booking_id = booking_pre_1 booking_supplement_id = booking_supplement_pre_1 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1 booking_id = booking_final_1 booking_supplement_id = booking_supplement_final_1 ) )
    ( preliminary = VALUE zjs_if_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1 booking_id = booking_pre_1 booking_supplement_id = booking_supplement_pre_2 )
    final = VALUE zjs_if_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1 booking_id = booking_final_1 booking_supplement_id = booking_supplement_final_2 ) )
    ).
